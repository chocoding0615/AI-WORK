...
// Shared orchestration for #003, used by BOTH the CLI (run.js) and the local
// web server (server.js). Not a generic automation interface — this is the
// one pipeline #003 has.
...
function runPipeline(csvText, { model = DEFAULT_MODEL } = {}) {
  ...
  try {
    interpretResult = callClaude({
      ...
      userPrompt: buildInterpretPrompt(candidates),
      model,
    });
  } catch (e) {
  ...
  let aiMap;
  try {
    aiMap = validateInterpretation(interpretResult.parsed, candidates.map((_, idx) => `c${idx}`));
  } catch (e) {
