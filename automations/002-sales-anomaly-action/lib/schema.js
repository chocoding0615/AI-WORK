...
const CANONICAL_ALIASES = {
  date: ['date', '날짜', '일자', '매출일자', '판매일자'],
  product: ['product', '상품', '상품명', '품목', '품목명', '제품명'],
  sales: ['sales', '매출', '매출액', '판매금액'],
  quantity: ['quantity', 'qty', '수량', '판매수량', '판매량'],
};

const FIELD_ORDER = ['date', 'product', 'sales', 'quantity'];
...
// Accept a few harmless date separator variants (this is a formatting
// difference, not semantic guessing) and reject anything else, including
// dates that don't actually exist on the calendar.
const DATE_PATTERNS = [
  /^(\d{4})-(\d{2})-(\d{2})$/,
  /^(\d{4})\/(\d{2})\/(\d{2})$/,
  /^(\d{4})\.(\d{2})\.(\d{2})$/,
...
// (e.g. "12만원" must fail, not be silently interpreted).
